    ...
    <div class="actions">
      <div class="combat-buttons" v-if="!winner">
        ...
        <button type="button" class="nes-btn is-success actions__btn" :class="{ 'is-disabled': (health.player1 >= 100) || tracker.playerHeal >= limit.heal || turnInProgress, 'focused': battleMenuIndex === 2 }" @click="$emit('heal')" @mouseover="$emit('hover-index', 2)">
          ...
          <div class="btn-subtext">{{ limit.heal - tracker.playerHeal }} Left</div>
        </button>
      </div>
      <div class="combat-buttons" v-else>
        <button class="nes-btn is-primary" :class="{ 'focused': battleMenuIndex === 0 }" @click="$emit('new')" @mouseover="$emit('hover-index', 0)">New Fighter <span class="key-hint">[ENTER]</span></button>
        ...
        <button class="nes-btn is-error" :class="{ 'focused': battleMenuIndex === 2 }" @click="$emit('exit')" @mouseover="$emit('hover-index', 2)">Exit <span class="key-hint">[ESC]</span></button>
      </div>
    </div>
  `,
  props: {
