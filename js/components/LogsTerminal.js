...
export default {
  ...
  template: `
    <div class="logs-terminal" role="log" aria-live="polite">
      <p v-for="(log, index) in logs" :key="index" :class="log.cls">
        <span v-if="log.icon" class="log-line-icon"><pixel-icon :name="log.icon" :size="10"></pixel-icon></span>{{ log.cls === 'log-round' ? log.text : '> ' + log.text }}
      </p>
